...
static bool pdelf_check_ehdr(Elf_X& elf) {
  ...
  return ((elf.e_type() == ET_EXEC || elf.e_type() == ET_DYN || elf.e_type() == ET_REL) &&
          (((elf.e_shoff() != 0) && (elf.e_shnum() != 0)) || ((elf.e_phoff() != 0) && (elf.e_phnum() != 0))));
}
...
extern template class Dyninst::SymtabAPI::emitElf<ElfTypes32>;

extern template class Dyninst::SymtabAPI::emitElf<ElfTypes64>;

set<string> debugInfoSections = list_of(string(SYMTAB_NAME))(string(STRTAB_NAME));

// loaded_elf(): populate elf section pointers
// for EEL rewritten code, also populate "code_*_" members
bool Object::loaded_elf(Offset& txtaddr, Offset& dataddr, Elf_X_Shdr*& bssscnp, Elf_X_Shdr*& symscnp,
                        Elf_X_Shdr*& strscnp, Elf_X_Shdr*& rel_plt_scnp, Elf_X_Shdr*& plt_scnp, Elf_X_Shdr*& got_scnp,
                        Elf_X_Shdr*& dynsym_scnp, Elf_X_Shdr*& dynstr_scnp, Elf_X_Shdr*& dynamic_scnp,
                        Elf_X_Shdr*& eh_frame, Elf_X_Shdr*& gcc_except, Elf_X_Shdr*& interp_scnp, Elf_X_Shdr*& opd_scnp,
                        Elf_X_Shdr*& symtab_shndx_scnp, bool) {
  ...
  for(auto i = 0UL; i < elfHdr->e_shnum(); ++i) {
    ...
    if(scn.sh_type() != SHT_NOTE && scn.sh_type() != SHT_NOBITS && (scn.sh_flags() & SHF_ALLOC)) {
      hasBitsAlloc_ = true;
    }
  ...
  for(unsigned int i = 0; i < elfHdr_numSections + elfHdrDbg_numSections; i++) {
    ...
    Elf_X_Shdr& scn = (!isFromDebugFile) ? elfHdr->get_shdr(i) : elfHdrDbg->get_shdr(i - elfHdr_numSections);
    if(!scn.isValid()) { // section is malformed
      continue;
    }
    Elf_X_Shdr* scnp = &scn;
    ...
    if(!scn.isFromDebugFile()) {
      ...
      if(elfHdr->e_machine() == EM_PPC || elfHdr->e_machine() == EM_PPC64) {
        ...
        if(strcmp(name, TEXT_NAME) == 0 || strcmp(name, ".rodata") == 0 || strcmp(name, INIT_NAME) == 0 ||
           strcmp(name, FINI_NAME) == 0 || (scn.sh_flags() & SHF_EXECINSTR)) {
      ...
      if(scn.sh_flags() & SHF_ALLOC) {
        ...
        Region* reg = new Region(i, name, scn.sh_addr(), diskSize, scn.sh_addr(), scn.sh_size(), ((char*)data.d_buf()),
                                 getRegionPerms(scn.sh_flags()), getRegionType(scn.sh_type(), scn.sh_flags(), name),
                                 true, ((scn.sh_flags() & SHF_TLS) != 0), scn.sh_addralign());
        reg->setFileOffset(scn.sh_offset());
        regions_.push_back(reg);
      } else {
        Region* reg = new Region(i, name, scn.sh_addr(), scn.sh_size(), 0, 0, ((char*)data.d_buf()),
                                 getRegionPerms(scn.sh_flags()), getRegionType(scn.sh_type(), scn.sh_flags(), name),
                                 false, ((scn.sh_flags() & SHF_TLS) != 0), scn.sh_addralign());
...
encompasses the data region. */
      ...
      eh_frame = scnp;
    } else if((strcmp(name, EXCEPT_NAME) == 0) || (strcmp(name, EXCEPT_NAME_ALT) == 0)) {
      gcc_except = scnp;
...
void Object::parseDynamic(Elf_X_Shdr*& dyn_scnp, Elf_X_Shdr*& dynsym_scnp, Elf_X_Shdr*& dynstr_scnp) {
...
bool Object::get_relocationDyn_entries(unsigned rel_scnp_index, Elf_X_Shdr*& dynsym_scnp, Elf_X_Shdr*& dynstr_scnp) {
  ...
  while(num_rel_entries_found != num_rel_entries) {
    ...
    for(const auto& reloc : relocation_table_) {
      std::cerr << reloc << '\n';
    }
  }
  return true;
    ...
    if(reldata.isValid() && symdata.isValid() && strdata.isValid()) {
      ...
      if(getArch() == Dyninst::Arch_x86 || getArch() == Dyninst::Arch_x86_64) {
        ...
        if(plt_entry_size_ == 8) {
          ...
          // New style secure PLT
          Region *plt = NULL, *dynamic = NULL, *got = NULL, *glink = NULL;
          ...
          for(unsigned iter = 0; iter < regions_.size(); ++iter) {
            ...
            if(name == PLT_NAME) {
              plt = regions_[iter];
            } else if(name == DYNAMIC_NAME) {
              dynamic = regions_[iter];
            } else if(name == GOT_NAME) {
              got = regions_[iter];
            }
          }
          ...
          if(dynamic != NULL) {
            ...
            for(unsigned int i = 0; i < count; ++i) {
              ...
              if(dyn[i].d_tag == DT_LOPROC) {
                ...
                if(got != NULL) {
                  unsigned char* data = (unsigned char*)got->getPtrToRawData();
                  glink_addr = Dyninst::read_memory_as<unsigned int>(data + (g_o_t - got->getMemOffset() + 4));
                  break;
                }
              }
            }
          ...
          auto insn = alignas_cast<unsigned int>(sec_data + (stub_addr - glink->getMemOffset()));

          // Keep moving pointer back if more -fPIC stubs are found.
          while(sec_data < (unsigned char*)insn) {
            unsigned int* back = insn - 4;

            if(((back[0] & 0xffff0000) == LWZ_11_30 && back[1] == MTCTR_11 && back[2] == BCTR)

               || ((back[0] & 0xffff0000) == ADDIS_11_30 && (back[1] & 0xffff0000) == LWZ_11_11 &&
                   back[2] == MTCTR_11 && back[3] == BCTR)) {
              extraStubs = true;
              stub_addr -= 16;
              insn = back;
            } else {
        ...
        for(iter = symbols_.begin(); iter != symbols_.end(); ++iter) {
          ...
          if(name.length() > 8) {
            ...
            relocationEntry re(stub_sym->getOffset(), pltrel_iter->second, name, targ_sym);
            fbt_.push_back(re);
          }
        }
      ...
      if(sym.isValid() && (rel.isValid() || rela.isValid()) && strs) {
        ...
        for(u_int i = 0; i < (rel_plt_size_ / rel_plt_entry_size_); ++i) {
          ...
          if(fbt_iter == -1) { // Create new relocation entry.
            ...
            relocationEntry re(next_plt_entry_addr, offset, targ_name, NULL, type);
...
void Object::load_object(bool alloc_syms) {
  ...
  { // binding contour (for "goto cleanup")
    ...
    if(!loaded_elf(txtaddr, dataddr, bssscnp, symscnp, strscnp, rel_plt_scnp, plt_scnp, got_scnp, dynsym_scnp,
                   dynstr_scnp, dynamic_scnp, eh_frame_scnp, gcc_except, interp_scnp, opd_scnp, symtab_shndx_scnp,
                   true)) {
      goto cleanup;
    }
    ...
    if(eh_frame_scnp != 0 && gcc_except != 0) {
      find_catch_blocks(eh_frame_scnp, gcc_except, txtaddr, dataddr, catch_addrs_);
    }
    ...
    if(alloc_syms) {
      ...
      parse_all_relocations(dynsym_scnp, dynstr_scnp, symscnp, strscnp);
...
Symbol* Object::handle_opd_symbol(Region* opd, Symbol* sym) {
  ...
  if(!opd->isOffsetInRegion(soffset)) {
    return NULL; // Symbol must be in .opd section.
  }
...
bool Object::parse_symbols(Elf_X_Data& symdata, Elf_X_Data& strdata, Elf_X_Shdr* bssscnp, Elf_X_Shdr* symscnp,
                           Elf_X_Shdr* symtab_shndx_scnp, bool /*shared*/) {
#if defined(TIMED_PARSE)
  struct timeval starttime;
  gettimeofday(&starttime, NULL);
#endif
...
#pragma omp for schedule(dynamic)
  ...
  */
        if(bssscnp) {
          ...
          if((bssStart <= soffset) && (soffset < bssEnd) && (ssize > 0) && (stype == Symbol::ST_NOTYPE)) {
        ...
        Symbol* newsym = new Symbol(sname, stype, slinkage, svisibility, soffset, NULL, sec, ssize, false,
                                    (secNumber == SHN_ABS), ind, strindex, (secNumber == SHN_COMMON));
        newsyms[i] = newsym;
...
void Object::parse_dynamicSymbols(Elf_X_Shdr*& dyn_scnp, Elf_X_Data& symdata, Elf_X_Data& strdata, bool /*shared*/) {
#if defined(TIMED_PARSE)
  struct timeval starttime;
  gettimeofday(&starttime, NULL);
#endif
  ...
  if(syms.isValid()) {
    for(unsigned i = 0; i < syms.count(); i++) {
      ...
      Symbol* newsym = new Symbol(sname, stype, slinkage, svisibility, soffset, NULL, sec, ssize,
                                  true, // is dynamic
                                  (secNumber == SHN_ABS), ind, strindex, (secNumber == SHN_COMMON));
...
bool Object::fix_global_symbol_modules_static_dwarf() {
  ...
  for(Dwarf_Off cu_off = 0, next_cu_off;
      dwarf_nextcu(dbg, cu_off, &next_cu_off, &cu_header_size, NULL, NULL, NULL) == 0; cu_off = next_cu_off) {
    ...
    const bool is_partialcu = DwarfDyninst::is_partial_unit(cu_die);
    const bool is_fullcu = DwarfDyninst::is_full_unit(cu_die);
  ...
  for(size_t i = 0; i < dies.size(); i++) {
    ...
    const auto loc = dwarf_dieoffset(&cu_die);
...
void Object::find_code_and_data(Elf_X& elf, Offset txtaddr, Offset dataddr) {
  /* Note:
  ...
  for(auto i = 0UL; i < elf.e_phnum(); ++i) {
    ...
    if(((phdr.p_vaddr() <= txtaddr) && (phdr.p_vaddr() + phdr.p_filesz() >= txtaddr)) ||
       (!txtaddr && ((phdr.p_vaddr() <= entryAddress_) && (phdr.p_vaddr() + phdr.p_filesz() >= entryAddress_)))) {
    ...
    } else if(((phdr.p_vaddr() <= dataddr) && (phdr.p_vaddr() + phdr.p_filesz() >= dataddr)) ||
...
Object::Object(MappedFile* mf_, bool, void (*err_func)(const char*), bool alloc_syms, Symtab* st)
    : AObject(mf_, err_func, st), elfHdr(NULL), hasReldyn_(false), hasReladyn_(false), hasRelplt_(false),
      hasRelaplt_(false), relType_(Region::RT_REL), hasNoteSection_(false), elf_hash_addr_(0), gnu_hash_addr_(0),
      dynamic_offset_(0), dynamic_size_(0), dynsym_size_(0), init_addr_(0), fini_addr_(0), text_addr_(0), text_size_(0),
      symtab_addr_(0), strtab_addr_(0), dynamic_addr_(0), dynsym_addr_(0), dynstr_addr_(0), got_addr_(0), got_size_(0),
      plt_addr_(0), plt_size_(0), plt_entry_size_(0), rel_plt_addr_(0), rel_plt_size_(0), rel_plt_entry_size_(0),
      rel_addr_(0), rel_size_(0), rel_entry_size_(0), opd_addr_(0), opd_size_(0), dwarvenDebugInfo(false),
      loadAddress_(0), entryAddress_(0), interpreter_name_(NULL), hasPieFlag_(false), hasDtDebug_(false),
      hasProgramLoad_(false), hasBitsAlloc_(false), hasDebugSections_(false), hasModinfo_(false),
      hasGnuLinkonceThisModule_(false), isStripped(false), dwarf(NULL), EEL(false), did_open(false),
      obj_type_(obj_Unknown), DbgSectionMapSorted(false), soname_(NULL), containingFunc(nullptr) {
  li_for_object = NULL;
...
void Object::get_valid_memory_areas(Elf_X& elf) {
  for(unsigned i = 0; i < elf.e_shnum(); ++i) {
    ...
    if(shdr.sh_flags() & SHF_ALLOC) { // This section is in memory
      if(code_off_ <= shdr.sh_addr() && shdr.sh_addr() <= code_off_ + code_len_) {
      ...
      } else if(data_off_ <= shdr.sh_addr() && shdr.sh_addr() <= data_off_ + data_len_) {
...
static int read_val_of_type(int type, unsigned long* value, const unsigned char* addr, const mach_relative_info& mi) {
  unsigned size = 0;
...
static int read_except_table_gcc3(const unsigned char e_ident[], mach_relative_info& mi, Elf_X_Shdr* eh_frame,
                                  Elf_X_Shdr* except_scn, std::vector<ExceptionBlock>& addresses) {
  Dwarf_Addr low_pc = 0;
  ...
  do {
    ...
    if(res == 1 && next_offset == (Dwarf_Off)-1) {
      break;
    }
    ...
    for(j = 0; j < augmentor_len; j++) {
      if(augmentor[j] == 'L') {
        ...
        cur_augdata += read_val_of_type(personality_encoding, &personality_val, cur_augdata, mi);
    ...
    const unsigned char* pc_begin_start =
        fde_bytes + 4 /* CIE Pointer size is 4 bytes */ +
    ...
    for(j = 0; j < augmentor_len; j++) {
      ...
      } else if(augmentor[j] == 'P' || augmentor[j] == 'z' || augmentor[j] == 'R') {
    ...
    while(except_off < table_end && except_off < except_size) {
      ...
      except_off += read_val_of_type(table_format, &region_start, datap + except_off, mi);
      mi.pc = except_scn->sh_addr() + except_off;
      tryEnd = except_off;
      except_off += read_val_of_type(table_format, &region_size, datap + except_off, mi);
      mi.pc = except_scn->sh_addr() + except_off;
      catchStart = except_off;
      except_off += read_val_of_type(table_format, &catch_block, datap + except_off, mi);
      except_off += read_val_of_type(DW_EH_PE_uleb128, &action, datap + except_off, mi);

      if(catch_block == 0) {
        continue;
      }
      ExceptionBlock eb(region_start + landingpad_base, (unsigned)region_size, catch_block + landingpad_base);
 ...
 **/
bool Object::find_catch_blocks(Elf_X_Shdr* eh_frame, Elf_X_Shdr* except_scn, Address txtaddr, Address dataaddr,
...
bool AObject::getSegments(vector<Segment>& segs) const {
  ...
  for(i = 0; i < regions_.size(); i++) {
    if((regions_[i]->getRegionName() == ".text") || (regions_[i]->getRegionName() == ".init") ||
       (regions_[i]->getRegionName() == ".fini") || (regions_[i]->getRegionName() == ".rodata") ||
       (regions_[i]->getRegionName() == ".plt") || (regions_[i]->getRegionName() == ".data")) {
      Segment seg{};
...
void Object::parseLineInfoForCU(Offset offset_, LineInformation* li_for_module) {
  ...
  for(size_t i = 0; i < lineCount; i++) {
    ...
    if(status != 0) {
      current_statement.column_number = 0;
    }
    ...
    if(current_line.uninitialized()) {
      ...
      if(!current_line.sameFileLineColumn(current_statement) || isEndOfSequence) {
        auto success = li_for_module->addLine(
            (unsigned int)(current_line.string_table_index), (unsigned int)(current_line.line_number),
            (unsigned int)(current_line.column_number), current_line.start_addr, current_line.end_addr);
        lineinfo_printf("[%lu, %lu) %s:%d %s\n", current_line.start_addr, current_line.end_addr,
                        ((*strings)[current_line.string_table_index]).str.c_str(), current_line.line_number,
                        (success ? " inserted" : " not"));
        current_line = current_statement;
...
void dumpLineWithInlineContext(open_statement& saved_statement, vector<open_statement>& inline_context) {
...
void Object::recordLine(Region* debug_str, open_statement& saved_statement, vector<open_statement>& inline_context) {
  ...
  li_for_object->addLine((unsigned int)(saved_statement.string_table_index),
                         (unsigned int)(saved_statement.line_number), (unsigned int)(saved_statement.column_number),
                         saved_statement.start_addr, saved_statement.end_addr);
  ...
  if(debug_str != nullptr && inline_context.size()) {
    ...
    if(containingFunc == nullptr || containingFunc->getOffset() >= saved_statement.start_addr ||
    ...
    for(unsigned int i = 0; i < inline_context.size() - 1; ++i) {
      cur = recordAnInlinedFunction(inline_context[i], inline_context[i + 1], strings, cur, saved_statement.start_addr,
                                    saved_statement.end_addr);
    }
    recordAnInlinedFunction(*(inline_context.rbegin()), saved_statement, strings, cur, saved_statement.start_addr,
                            saved_statement.end_addr);
  }
...
InlinedFunction* Object::recordAnInlinedFunction(open_statement& caller, open_statement& callee, StringTablePtr strings,
                                                 FunctionBase* parent, Dwarf_Addr start, Dwarf_Addr end) {
  ...
  ifunc->callsite_file_number =
      strings->project<0>(strings->get<1>().insert(StringTableEntry(src_file, "")).first) - strings->begin();
...
void Object::lookupInlinedContext(vector<open_statement>& inline_context, open_statement& saved_statement) {
...
LineInformation* Object::parseLineInfoForObject(StringTablePtr strings) {
  ...
  while(1) {
    ...
    status = dwarf_next_lines(dbg, off = next_off, &next_off, &cu, &files, &fileCount, &lineBuffer, &lineCount);
...
void Object::parseTypeInfo() {
  ...
  unsigned long difftime = lendtime - lstarttime;
  double dursecs = difftime / (1000);
  cout << __FILE__ << ":" << __LINE__ << ": parseTypes(" << obj->file() << ") took " << dursecs << " msecs" << endl;
#endif
}
...
bool Region::isStandardCode() {
  return ((getRegionPermissions() == RP_RX || getRegionPermissions() == RP_RWX) &&
          ((name_ == std::string(".text")) || (name_ == std::string(".init")) || (name_ == std::string(".fini"))));
}
...
bool Object::isLoadable() const {
  return (objType() == obj_Executable || objType() == obj_SharedLib) && hasProgramLoad() &&
...
bool Object::isDebugOnly() const {
  ...
  return (type == obj_RelocatableFile || type == obj_Executable || type == obj_SharedLib) &&
         (hasDebugSections() || getSymtabAddr()) && !hasBitsAlloc();
}
...
bool Object::isLinuxKernelModule() const {
  return objType() == obj_RelocatableFile && hasModinfo() && hasGnuLinkonceThisModule();
}
