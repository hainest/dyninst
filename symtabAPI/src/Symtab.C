...
boost::shared_ptr<typeCollection> Symtab::setupStdTypes() {
  boost::shared_ptr<typeCollection> stdTypes = boost::shared_ptr<typeCollection>(new typeCollection);
...
DYNINST_EXPORT Symtab::Symtab()
    : LookupInterface(), AnnotatableSparse(), impl{std::unique_ptr<symtab_impl>(new symtab_impl{})} {
...
DYNINST_EXPORT void Symtab::fixup_code_and_data(Offset newImageOffset, Offset newImageLength, Offset newDataOffset,
...
void Symtab::setModuleLanguages(dyn_hash_map<std::string, supportedLanguages>* mod_langs) {
  ...
  for(auto* currmod : impl->modules) {
    ...
    } else if(fn.rfind(".s") != std::string::npos || fn.rfind(".asm") != std::string::npos) {
      currLang = lang_Assembly;
    } else if(fn.rfind(".c") != std::string::npos) {
      currLang = lang_C;
    } else if(fn.rfind(".cpp") != std::string::npos || fn.rfind(".cc") != std::string::npos ||
...
void Symtab::createDefaultModule() {
  Module* mod = new Module(lang_Unknown, imageOffset_, file(), this);
...
Module* Symtab::getOrCreateModule(const std::string& modName, const Offset modAddr) {
...
Symtab::Symtab(unsigned char* mem_image, size_t image_size, const std::string& name, bool defensive_bin, bool& err)
    : Symtab() {
  isDefensiveBinary_ = defensive_bin;
  // Initialize error parameter
  err = false;
  ...
  for(unsigned index = 0; index < regions_.size(); index++) {
    ...
    if(regions_[index]->isLoadable()) {
      if((regions_[index]->getRegionPermissions() == Region::RP_RX) ||
         (isDefensiveBinary_ && regions_[index]->getRegionPermissions() == Region::RP_RW) ||
...
bool Symtab::isCode(const Offset where) const {
  ...
  while(last >= first) {
    ...
    if(where >= curreg->getMemOffset() && where < (curreg->getMemOffset() + curreg->getMemSize())) {
...
DYNINST_EXPORT bool Symtab::findPltEntryByTarget(const Address target_address, relocationEntry& result) const {
   ...
   * linker
   */
  if(relocation_table_.empty() && !isStaticBinary() && getObjectType() != obj_RelocatableFile) {
    fprintf(stderr, "%s[%d]:  WARN:  zero func bindings\n", FILE__, __LINE__);
  }

  auto it = std::find_if(relocation_table_.cbegin(), relocation_table_.cend(), [=](const relocationEntry& entry) {
...
bool Symtab::openFile(Symtab*& obj, void* mem_image, size_t size, std::string name, def_t def_bin) {
  bool err = false;
#if defined(TIMED_PARSE)
  struct timeval starttime;
  gettimeofday(&starttime, NULL);
...
bool Symtab::addRegion(Offset vaddr, void* data, unsigned int dataSize, std::string name, Region::RegionType rType_,
  ...
  if(loadable) {
    ...
    sec = new Region(regions_.size() + 1, name, vaddr, dataSize, 0, 0, (char*)data, Region::RP_R, rType_, loadable, tls,
                     memAlign);
    sec->setSymtab(this);
...
DYNINST_EXPORT bool Symtab::getAddressRanges(std::vector<AddressRange>& ranges, std::string lineSource,
                                             unsigned int lineNo) {
...
DYNINST_EXPORT Offset Symtab::getFreeOffset(unsigned size) {
  ...
  for(unsigned i = 0; i < regions_.size(); i++) {
    ...
    unsigned region_offset = (unsigned)((char*)(regions_[i]->getPtrToRawData()) - linkedFile->mem_image());
    ...
    if((i < (regions_.size() - 2)) && ((end + size) < regions_[i + 1]->getMemOffset())) {
      newSectionInsertPoint = i + 1;
      highWaterMark = end;
      break;
    }
...
bool Symtab::getRegValueAtFrame(Address pc, Dyninst::MachRegister reg, Dyninst::MachRegisterVal& reg_result,
                                MemRegReader* reader) {
  Object* obj = getObject();
  if(!obj) {
    return false;
...
void Symtab::parseTypesNow() {
  std::call_once(this->impl->types_parsed, [this]() {
    this->parseTypes();
  });
}
...
DYNINST_EXPORT bool Symtab::addExternalSymbolReference(Symbol* externalSym, Region* localRegion,
  ...
  Symbol* symRef = new Symbol(externalSym->getMangledName(), externalSym->getType(), Symbol::SL_GLOBAL,
