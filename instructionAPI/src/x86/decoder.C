...
namespace Dyninst { namespace InstructionAPI {
    ...
    void x86_decoder::decode_operands(Instruction const *insn) {
      ...
      for(auto r : implicit_registers(d)) {
        ...
        if(this->mode == CS_MODE_64) {
          ...
          if(insn->getOperation().getID() == e_lcall) {
            if(reg == X86_REG_EIP) {
              reg = X86_REG_RIP;
        ...
        if(reg == X86_REG_EFLAGS || reg == X86_REG_FPSW) {
          ...
          // Capstone doesn't track implicit writes to/through the stack pointer
          auto const sp_reg = x86::translate_register(reg, this->mode);
          auto const type = size_to_type_unsigned(disassembler.insn->detail->x86.addr_size);
    ...
    void x86_decoder::decode_imm(Instruction const *insn, cs_x86_op const &operand) {
      auto const size = disassembler.insn->detail->x86.encoding.imm_size;
