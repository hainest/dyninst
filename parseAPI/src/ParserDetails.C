...
static void getBlockInsns(Block& blk, std::set<Address>& addrs) {
  ...
  const unsigned char* bufferBegin = (const unsigned char*)(blk.region()->getPtrToInstruction(blk.start()));
  InstructionDecoder dec = InstructionDecoder(bufferBegin, bufSize, blk.region()->getArch());
  InstructionAdapter_t* ah = InstructionAdapter_t::makePlatformIA_IAPI(blk.obj()->cs()->getArch(), dec, blk.start(),
                                                                       blk.obj(), blk.region(), blk.obj()->cs(), &blk);
...
ParseFrame* Parser::getTamperAbsFrame(Function* tamperFunc) {
  ...
  targFunc = _parse_data->createAndRecordFunc(*(targRegs.begin()), target, tamperFunc->src());
...
void Parser::tamper_post_processing(LockFreeQueue<ParseFrame*>& work_queue, ParseFrame* pf) {
  ...
  for(auto widx = work.begin(); pf->func->tampersStack() == TAMPER_REL && widx != work.end(); widx++) {
    if((*widx)->status() == ParseFrame::CALL_BLOCKED && pf->func == (*widx)->call_target) {
      for(unsigned bidx = 0; bidx < (*widx)->work_bundles.size(); bidx++) {
        const vector<ParseWorkElem*>& elems = (*widx)->work_bundles[bidx]->elems();
        ...
        for(unsigned eix = 0; eix < elems.size(); eix++) {
          ...
          if(elems[eix]->edge()->type() == CALL && elems[eix]->target() == pf->func->addr()) {
            rightBundle = true;
 ...
 */
inline void Parser::ProcessUnresBranchEdge(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah, Address target) {
 ...
 */
inline void Parser::ProcessReturnInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah) {
 ...
 */
inline void Parser::ProcessCallInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah, bool isDynamic,
                                    bool isAbsolute, bool isResolved, Address target) {
  ParseCallback::interproc_details det;
  det.ibuf = (unsigned char*)frame.func->isrc()->getPtrToInstruction(ah->getAddr());
  det.isize = ah->getSize();
...
bool Parser::ProcessCFInsn(ParseFrame& frame, Block* cur, InstructionAdapter_t* ah) {
  ...
  if(unlikely(_obj.defensiveMode() && !ah->isCall() && edges_out.size())) {
    ...
    for(Edges_t::iterator curEdge = edges_out.begin(); !hasUnalignedEdge && curEdge != edges_out.end(); ++curEdge) {
      ...
      if(!tregs.empty()) {
        ...
        for(set<Block*>::iterator bit = tblocks.begin(); !hasUnalignedEdge && bit != tblocks.end(); bit++) {
    ...
    if(true == hasUnalignedEdge) {
      ...
      edges_out.clear();
      ParseCallback::default_details det((unsigned char*)cur->region()->getPtrToInstruction(cur->lastInsnAddr()),
  ...
  for(Edges_t::iterator curEdge = edges_out.begin(); curEdge != edges_out.end(); ++curEdge) {
    Edge* newedge = NULL;
    bool resolvable_edge = true;
    bool tailcall = false;
    parsing_printf("\t\t\tedge target %lx, is_code %d, is_plt %d, edge type %d, dynamic_call %d\n", curEdge->first,
                   is_code(frame.func, curEdge->first), HASHDEF(plt_entries, curEdge->first), curEdge->second,
                   dynamic_call);
    ...
    if(curEdge->second == CALL) {
      ...
      ProcessCallInsn(frame, cur, ah, dynamic_call, absolute_call, resolvable_edge, curEdge->first);
    ...
    if(ah->isTailCall(frame.func, curEdge->second, frame.num_insns, frame.knownTargets)) {
      ...
      parsing_printf("Setting edge 0x%p (0x%lx/0x%lx) to interproc (tail call)\n", (void*)newedge,
                     newedge->src()->start(), newedge->trg_addr());
      newedge->_type._interproc = true;
    }
    ...
    ParseWorkElem* we =
        bundle->add(new ParseWorkElem(bundle, newedge, ah->getAddr(), curEdge->first, resolvable_edge, tailcall));
    ...
    if(resolvable_edge) {
      ...
      if(unlikely(_obj.defensiveMode())) {
        ...
        if(CALL == curEdge->second || DIRECT == curEdge->second || COND_TAKEN == curEdge->second) {
