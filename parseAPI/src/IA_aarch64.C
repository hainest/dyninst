...
bool IA_aarch64::savesFP() const {
  ...
  // stp x29, x30, [sp, imm]!
  if(insn.getOperation().getID() == aarch64_op_stp_gen && insn.isRead(framePtr[_isrc->getArch()]) &&
     insn.isRead(returnAddrReg) && insn.isRead(stackPtr[_isrc->getArch()]) &&
...
bool IA_aarch64::cleansStack() const {
  ...
  // ldp x29, x30, [sp], imm
  if(insn.getOperation().getID() == aarch64_op_ldp_gen && insn.isWritten(framePtr[_isrc->getArch()]) &&
     insn.isWritten(returnAddrReg) && insn.isRead(stackPtr[_isrc->getArch()]) &&
     insn.isWritten(stackPtr[_isrc->getArch()])) {
    return true;
  }
