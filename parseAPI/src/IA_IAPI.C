...
IA_IAPI::IA_IAPI(const IA_IAPI& rhs)
    : InstructionAdapter(rhs), dec(rhs.dec), allInsns(rhs.allInsns), validCFT(rhs.validCFT), cachedCFT(rhs.cachedCFT),
      validLinkerStubState(rhs.validLinkerStubState), cachedLinkerStubState(rhs.cachedLinkerStubState),
      hascftstatus(rhs.hascftstatus), tailCalls(rhs.tailCalls) {
...
IA_IAPI* IA_IAPI::makePlatformIA_IAPI(Architecture arch, InstructionDecoder dec_, Address where_, CodeObject* o,
                                      CodeRegion* r, InstructionSource* isrc, Block* curBlk_) {
  switch(arch) {
    case Arch_x86:
    case Arch_x86_64:
...
void IA_IAPI::initASTs() {
  std::call_once(IA_IAPI::ptrInit, [&] {
    if(framePtr.empty()) {
      ...
      framePtr[Arch_amdgpu_gfx908] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getFramePointer(Arch_amdgpu_gfx908)));
      framePtr[Arch_amdgpu_gfx90a] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getFramePointer(Arch_amdgpu_gfx90a)));
      framePtr[Arch_amdgpu_gfx940] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getFramePointer(Arch_amdgpu_gfx940)));
    }
    if(stackPtr.empty()) {
      ...
      stackPtr[Arch_amdgpu_gfx908] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getStackPointer(Arch_amdgpu_gfx908)));
      stackPtr[Arch_amdgpu_gfx90a] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getStackPointer(Arch_amdgpu_gfx90a)));
      stackPtr[Arch_amdgpu_gfx940] =
          RegisterAST::Ptr(new RegisterAST(MachRegister::getStackPointer(Arch_amdgpu_gfx940)));
    }
    if(thePC.empty()) {
...
IA_IAPI::IA_IAPI(InstructionDecoder dec_, Address where_, CodeObject* o, CodeRegion* r, InstructionSource* isrc,
                 Block* curBlk_)
    : InstructionAdapter(where_, o, r, isrc, curBlk_), dec(dec_), validCFT(false), cachedCFT(std::make_pair(false, 0)),
      validLinkerStubState(false), cachedLinkerStubState(false) {
  hascftstatus.first = false;
  tailCalls.clear();

  curInsnIter = allInsns.insert(allInsns.end(), std::make_pair(current, dec.decode()));

  initASTs();
}

void IA_IAPI::reset(InstructionDecoder dec_, Address start, CodeObject* o, CodeRegion* r, InstructionSource* isrc,
                    Block* curBlk_) {
  // reset the base
...
bool IA_IAPI::isAbort() const {
  ...
  return e == e_int3 || e == e_hlt || e == e_ud2;
}
...
bool IA_IAPI::isGarbageInsn() const {
  ...
  if(unlikely(_obj->defensiveMode())) {
    ...
    switch(e) {
      case e_arpl:
        cerr << "REACHED AN ARPL AT " << std::hex << current << std::dec << " COUNTING AS INVALID" << endl;
        ret = true;
        break;
      case e_fisub:
        cerr << "REACHED A FISUB AT " << std::hex << current << std::dec << " COUNTING AS INVALID" << endl;
        ret = true;
        break;
      case e_into:
        cerr << "REACHED AN INTO AT " << std::hex << current << std::dec << " COUNTING AS INVALID" << endl;
        ret = true;
        break;
      case e_mov: {
        set<RegisterAST::Ptr> regs;
        curInsn().getWriteSet(regs);
        for(set<RegisterAST::Ptr>::iterator rit = regs.begin(); rit != regs.end(); rit++) {
          if(Dyninst::isSegmentRegister((*rit)->getID().regClass())) {
            cerr << "REACHED A MOV SEGMENT INSN AT " << std::hex << current << std::dec << " COUNTING AS INVALID"
                 << endl;
            ret = true;
            break;
          }
      ...
      case e_add:
        if(2 == curInsn().size() && 0 == curInsn().rawByte(0) && 0 == curInsn().rawByte(1)) {
          cerr << "REACHED A 0x0000 INSTRUCTION " << std::hex << current << std::dec << " COUNTING AS INVALID" << endl;
          ret = true;
        }
        break;
      case e_push: // pushes of segment registers do not occur frequently in real code (and crash Rose)
        switch(curInsn().rawByte(0)) {
          ...
          case 0x1e:
            ret = true;
            cerr << "REACHED A PUSH OF A SEGMENT REGISTER " << std::hex << current << std::dec << " COUNTING AS INVALID"
                 << endl;
            break;
          case 0x0f:
            if(2 == curInsn().size() && ((0xa0 == curInsn().rawByte(1)) || (0xa8 == curInsn().rawByte(1)))) {
              ret = true;
              cerr << "REACHED A 2-BYTE PUSH OF A SEGMENT REGISTER " << std::hex << current << std::dec
                   << " COUNTING AS INVALID" << endl;
            }
            break;
          default:
            break;
...
void IA_IAPI::getNewEdges(std::vector<std::pair<Address, EdgeTypeEnum>>& outEdges, Function* context, Block* currBlk,
                          unsigned int num_insns, dyn_hash_map<Address, std::string>* plt_entries,
  ...
  if(ci.isReturn()) {
    ...
    if(ci.allowsFallThrough()) {
      outEdges.push_back(std::make_pair(getCFT().second, COND_TAKEN));
...
std::pair<bool, Address> IA_IAPI::getCFT() const {
  ...
  if(actualTarget.defined) {
    ...
    parsing_printf("FAIL (CFT=0x%lx), callTarget exp: %s\n", cachedCFT.second,
                   callTarget->format(curInsn().getArch()).c_str());
  }
  validCFT = true;
...
bool IA_IAPI::parseJumpTable(Dyninst::ParseAPI::Function* currFunc, Dyninst::ParseAPI::Block* currBlk,
