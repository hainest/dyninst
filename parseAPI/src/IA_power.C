...
IA_power::IA_power(Dyninst::InstructionAPI::InstructionDecoder dec_, Address start_, Dyninst::ParseAPI::CodeObject* o,
                   Dyninst::ParseAPI::CodeRegion* r, Dyninst::InstructionSource* isrc,
...
bool IA_power::isTailCall(const Function* context, EdgeTypeEnum type, unsigned int,
  ...
  if(curInsn().isBranch() && valid && callee && callee != context &&
  ...
  if(curInsn().isBranch() && valid && !callee) {
...
class PPCReturnPredicates : public Slicer::Predicates {
  virtual bool widenAtPoint(Assignment::Ptr p) {
    for(std::vector<AbsRegion>::const_iterator iter = p->inputs().begin(); iter != p->inputs().end(); ++iter) {
...
bool IA_power::sliceReturn(ParseAPI::Block* bit, Address ret_addr, ParseAPI::Function* func) const {
  ...
  InstructionDecoder retdec(_isrc->getPtrToInstruction(retnAddr), InstructionDecoder::maxInstructionLength,
                            _cr->getArch());
...
bool IA_power::isReturnAddrSave(Address& retAddr) const {
  ...
  if(ci.getOperation().getID() == power_op_mfspr && ci.isRead(regLR)) {
    foundMFLR = true;
    ci.getWriteSet(regs);
    if(regs.size() != 1) {
  ...
  if(foundMFLR) {
    ...
    while(!copy.hasCFT()) {
      ci = copy.curInsn();
      if(ci.writesMemory() && ci.isRead(regSP) && ci.isRead(destLRReg)) {
        ret = true;
        break;
...
bool IA_power::isReturn(Dyninst::ParseAPI::Function* context, Dyninst::ParseAPI::Block* currBlk) const {
  ...
  parsing_printf("isblrReturn at 0x%lx Addr 0x%lx 0x%lx Function addr 0x%lx leaf %d \n", current, getAddr(),
                 currBlk->start(), context->addr(), func->_is_leaf_function);
  if(!func->_is_leaf_function) {
    ...
    const unsigned char* b = (const unsigned char*)(this->_isrc->getPtrToInstruction(blockStart));
    InstructionDecoder decCopy(b, currBlk->size(), this->_isrc->getArch());
    ...
    for(iter = copy.allInsns.rbegin(); iter != copy.allInsns.rend(); iter++) {
      ...
      if(iter->second.getOperation().getID() == power_op_mtspr && iter->second.isWritten(regLR)) {
        ...
        foundMTLR = true;
      } else if(foundMTLR && iter->second.readsMemory() &&
                (iter->second.isRead(regSP) || (iter->second.isRead(reg11))) && iter->second.isWritten(sourceLRReg)) {
        parsing_printf("\t\t\t **** Found lwz - RETURNING TRUE\n");
        return true;
      }
    }

    parsing_printf(" Slicing for Addr 0x%lx startAddr 0x%lx ret addr 0x%lx func %s\n", getAddr(), currBlk->start(),
                   func->_ret_addr, func->name().c_str());
...
AST::Ptr PPC_BLR_Visitor::visit(DataflowAPI::VariableAST* v) {
  if((v->val().reg == AbsRegion(ppc32::lr)) && (v->val().addr == ret_)) {
    return_ = PPC_BLR_RETURN;
  }
  // Check the stack
  else if((v->val().reg.type() == Absloc::Unknown) && (v->val().reg.absloc().type() == Absloc::Stack) &&
