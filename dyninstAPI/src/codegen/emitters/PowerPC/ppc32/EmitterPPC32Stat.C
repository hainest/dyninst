...
#include "codegen/emitters/PowerPC/ppc32/EmitterPPC32Stat.h"
#include "debug.h"
...
namespace Dyninst { namespace DyninstAPI {
  ...
  bool EmitterPPC32Stat::emitCallInstruction(codeGen &gen, func_instance *callee,
                                             bool /* setTOC */, Address) {
    // 32 - No TOC
  ...
  Register EmitterPPC32Stat::emitCallReplacement(opCode, codeGen &, func_instance *) {
    assert(0 && "emitCallReplacement not implemented for binary rewriter");
    return {};
  }

  // TODO 32/64-bit?
  bool EmitterPPC32Stat::emitPLTCall(func_instance *callee, codeGen &gen) {
    return emitPLTCommon(callee, true, gen);
  }

  bool EmitterPPC32Stat::emitPLTCommon(func_instance *callee, bool call, codeGen &gen) {
  ...
  bool EmitterPPC32Stat::emitPLTJump(func_instance *callee, codeGen &gen) {
    return emitPLTCommon(callee, false, gen);
  }

  bool EmitterPPC32Stat::emitTOCCall(block_instance *block, codeGen &gen) {
    return emitTOCCommon(block, true, gen);
  }

  bool EmitterPPC32Stat::emitTOCCommon(block_instance *block, bool call, codeGen &gen) {
    Dyninst::Register scratchReg = gen.rs()->getScratchRegister(gen);
    if (scratchReg == Null_Register) {
      return false;
    }
  ...
  bool EmitterPPC32Stat::emitTOCJump(block_instance *block, codeGen &gen) {
