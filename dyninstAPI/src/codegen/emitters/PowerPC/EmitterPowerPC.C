...
#include "emitters/PowerPC/generators.h"
...
namespace Dyninst { namespace DyninstAPI {
  ...
  codeBufIndex_t EmitterPowerPC::emitA(opCode op, Dyninst::Register src1, long dest,
  ...
  void EmitterPowerPC::emitAddrSpecLoad(const BPatch_addrSpec_NP *as,
                                        Dyninst::Register dest, int stackShift,
                                        codeGen &gen) {
  ...
  bool EmitterPowerPC::emitCallInstruction(codeGen &gen, func_instance *callee,
    ...
    if (needLongBranch) {
      ...
      inst_printf("[EmitterPowerPC::EmitCallInstruction] needLongBranch, Emitting VLOAD  "
                  "Callee: 0x%lx, "
                  "ScratchReg: %u\n",
    ...
    // Linux 64
    if (setTOC) {
      inst_printf("[EmitterPowerPC::EmitCallInstruction] Setting TOC anchor, toc_anchor: "
                  "%lx, register: "
                  "2(fixed)\n",
                  (uint64_t)toc_anchor);
    ...
    if (needLongBranch) {
      ...
      inst_printf("[EmitterPowerPC::EmitCallInstruction] Generating Call, curAddress: "
                  "%lx, calleeAddr: "
                  "%lx\n",
  ...
  void EmitterPowerPC::emitCallWithSaves(codeGen &gen, Address dest, bool saveToc,
                                         bool saveLR, bool saveR12) {
  ...
  void EmitterPowerPC::emitCountSpecLoad(const BPatch_addrSpec_NP *as,
  ...
  codeBufIndex_t EmitterPowerPC::emitIf(Register, Register,
                                        Dyninst::DyninstAPI::RegControl, codeGen &) {
    assert(0);
    return 0;
  }
  ...
  void EmitterPowerPC::emitLoadOrigRegRelative(Register, Address, Register, codeGen &,
                                               bool) {
    assert(0);
  }
  ...
  bool EmitterPowerPC::emitMoveRegToReg(registerSlot *src, registerSlot *dest,
                                        codeGen &gen) {
  ...
  void EmitterPowerPC::emitOpImm(unsigned, unsigned, Register, Register, RegValue,
                                 codeGen &) {
    assert(0);
  }
  ...
  Dyninst::Register EmitterPowerPC::emitR(opCode op, Dyninst::Register src1,
    ...
    case getParamOp: {
      ...
      if (src1 < 8) {
        ...
        if (addrWidth == 4) {
          stkOffset =
              TRAMP_FRAME_SIZE_32 + (src1 - 8) * sizeof(int) + PARAM_OFFSET(addrWidth);
        } else {
          // Linux ABI says:
          ...
          int stackSlot = src1;
          stkOffset =
              TRAMP_FRAME_SIZE_64 + stackSlot * sizeof(long) + PARAM_OFFSET(addrWidth);
        }
  ...
  void EmitterPowerPC::emitRelOp(unsigned, Register, Register, Register, codeGen &,
                                 bool) {
    assert(0);
  }

  void EmitterPowerPC::emitRelOpImm(unsigned, Register, Register, RegValue, codeGen &,
                                    bool) {
    assert(0);
  }
  ...
  void EmitterPowerPC::emitStoreFrameRelative(Address, Register, Register, int,
                                              codeGen &) {
    assert(0);
  }
  ...
  void EmitterPowerPC::emitStoreShared(Dyninst::Register source,
                                       const image_variable *var, bool is_local, int size,
                                       codeGen &gen) {
    // create or retrieve jump slot
    Address addr;
  ...
  void EmitterPowerPC::emitVstore(opCode op, Dyninst::Register src1,
                                  Dyninst::Register /*src2*/, Address dest, codeGen &gen,
                                  int size, AddressSpace *proc) {
    if (op == storeOp) {
