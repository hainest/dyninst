...
int main(int argc, char **argv) {
  ...
  // std::cout << vars[0]->getType() << ' ' << vars[0]->getOffset() << ' ' <<
  // *vars[0] << std::endl;
...
void print_type(Type *t) {
  ...
  case dataStructure: {
    ...
    std::for_each(fields->begin(), fields->end(), [](const auto &p) {
      std::cout << "\t\t" << p->getName() << ' ' << p->getOffset() << std::endl;
...
void print_loclists(std::vector<VariableLocation> &ll) {
  ...
  std::for_each(ll.begin(), ll.end(), [](const auto &p) {
    std::cout << "\t\t" << std::hex << p.lowPC << ' ' << p.hiPC << std::dec;
    const char *sclass;
    switch (p.stClass) {
    case storageUnset:
      sclass = "Unset";
    ...
    case storageReg:
      ...
      break;
    case storageRegOffset:
      std::cout << indirect << '[' << "fp" << (p.frameOffset > 0 ? "+" : "")
                << p.frameOffset << ']';
      break;
    default:
      exit(6);
...
void do_dump_globals(Symtab *obj) {
  ...
  auto print_vars = [](const auto &p) {
    std::for_each(p->pretty_names_begin(), p->pretty_names_end(),
                  [](const auto &a) { std::cout << a << ' '; });
    std::cout << ": " << std::hex << p->getOffset() << std::dec << ' '
              << p->getSize() << "b ";
    if (p->getType()) {
      std::cout << "\n\tType:";
      print_type(p->getType());
  ...
  std::for_each(clines.begin(), clines.end(), [&print_vars](const auto &v) {
    if (!v.empty()) {
      std::cout << std::endl;
...
void do_dump_functions(Symtab *obj) {
  ...
  auto print_funcs = [](const auto &p) {
    std::cout << *(p->typed_names_begin()) << '\t' << p->getName() << std::endl;
  };
...
void map_type_to_vars(Symtab *obj, std::multimap<Type *, localVar *> &map) {
  ...
  for (auto i = funcs.begin(); i != funcs.end(); i++) {
    ...
    std::for_each(lvars.begin(), lvars.end(), [&map](const auto &p) {
      map.insert(std::make_pair(p->getType(), p));
    });
  }
}
...
static void insert_types(std::set<Type *> &types, Type *p) {
  ...
  case dataStructure: {
    ...
    std::for_each(members->begin(), members->end(), [&types](const auto &mem) {
...
static void build_type_list(Symtab *obj, std::set<Type *> &types) {
  ...
  if (!obj->getAllVariables(vars))
    exit(EXIT_GLOBALS);
  std::for_each(vars.begin(), vars.end(), [&types](const auto &p) {
    if (p->getType())
  ...
  if (!obj->getAllFunctions(funcs))
    exit(EXIT_NOFUNCS);
  auto accumulate_types = [&types](const auto &fn) {
    std::vector<localVar *> lvars;
    ...
    std::for_each(lvars.begin(), lvars.end(), [&types](const auto &p) {
      if (p->getType())
        insert_types(types, p->getType());
    });
  };
...
void do_dump_types(Symtab *obj) {
  ...
  std::for_each(types.begin(), types.end(), [](const auto &p) {
...
void do_pahole(Symtab *obj) {
  ...
  for (auto i = types.begin(); i != types.end();) {
    ...
    std::cout << "class " << (*i)->getName() << ' ' << " {" << std::endl
              << "  // Size: " << (*i)->getSize()
              << ", Cachelines: " << ((*i)->getSize() >> CACHELINE_BITS) + 1
              << ", Members: " << members->size() << std::endl
              << std::endl;
