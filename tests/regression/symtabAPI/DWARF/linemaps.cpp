...
static bool read_file(std::map<string, vector<line_data>> &file_lines,
  ...
  ifstream inf(f);
  if (inf.fail()) {
    cerr << "Error: Problem reading source file " << f
         << ". Skipping:" << strerror(errno) << endl;
    inf.close();
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt_long(argc, argv, "vwqm", long_options, &option_index)) !=
         -1) {
    switch (opt) {
    case 'v':
      verbose = true;
  ...
  for (auto i : funcs) {
    if (!i->getModule()->fullName().empty()) {
      cerr << "f " << i->getName()
           << " Inserting: " << i->getModule()->fullName() << endl;
    ...
    for (auto j : lvars) {
      ...
      for (auto k : lvlocs) {
        ...
        if (k.lowPC < i->getOffset() ||
            k.hiPC > i->getOffset() + i->getSize()) {
        ...
        discrete_interval<Address> addr_inter =
            construct<discrete_interval<Address>>(k.lowPC, k.hiPC,
  ...
  for (auto i : funcs) {
    ...
    for (auto j : lvars) {
      ...
      if (i->getModule()->fullName() != j->getFileName()) {
        string basename =
            j->getFileName().substr(j->getFileName().rfind('/') + 1);
