...
namespace rose { namespace BinaryAnalysis { namespace InstructionSemantics2 {
...
#define EXTR(lo, hi) IntegerOps::extract2<B>(lo, hi, raw)
  ...
  namespace ARM64 {
    ...
    void InsnProcessor::process(const BaseSemantics::DispatcherPtr& dispatcher_, SgAsmInstruction* insn_) {
    ...
    struct IP_adc_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result = d->doAddOperation(operand1, operand2, false, d->readRegister(d->REG_C) /*, ops->boolean_(false)*/, n,
                                   z, c, v);
    ...
    struct IP_adr_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        if((EXTR(31, 31) == 1)) {
          base = ops->or_(ops->and_(base, ops->number_(64, 0xfffffffffffff000)), d->Zeros(12));
        }
    ...
    struct IP_b_cond_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        d->BranchTo(ops->ite(ops->isEqual(d->ConditionHolds(ops->number_(4, EXTR(0, 4))), ops->boolean_(true)),
    ...
    struct IP_cbz_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        d->BranchTo(ops->ite(ops->isEqual(d->isZero(operand1), ops->boolean_(EXTR(24, 24) == 0)), d->read(args[1]),
                             d->readRegister(d->REG_PC)));
      }
    };
    ...
    struct IP_tbz_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        d->BranchTo(ops->ite(ops->isEqual(ops->and_(ops->shiftRight(operand, d->read(args[1])), ops->number_(1, 1)),
    ...
    struct IP_ccmn_reg_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        operand2 = ops->ite(ops->isEqual(d->ConditionHolds(d->read(args[3])), ops->boolean_(true)),
                            (EXTR(30, 30) == 1 ? (carry_in = true, d->NOT(operand2)) : operand2), operand2);
    ...
    // TODO: For ubfm/sbfm based instructions, update the grammar to declare variables for imms/immr and wmask/tmask
    // instead of reading/calculating them twice
    struct IP_ubfm_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        BaseSemantics::SValuePtr tmask =
            d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), false, (EXTR(31, 31) + 1) * 32);
        BaseSemantics::SValuePtr wmask =
            d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), true, (EXTR(31, 31) + 1) * 32);
        ...
        BaseSemantics::SValuePtr bot =
            ops->or_(ops->and_(dst, d->NOT(wmask)), ops->and_(d->ROR(src, d->read(args[2])), wmask));
    ...
    struct IP_uxtb_ubfm_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        /*BaseSemantics::SValuePtr tmask = d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15), EXTR(22, 22), false, (EXTR(31,
           31) + 1) * 32); BaseSemantics::SValuePtr wmask = d->getBitfieldMask(EXTR(16, 21), EXTR(10, 15), EXTR(22, 22),
           true, (EXTR(31, 31) + 1) * 32); BaseSemantics::SValuePtr dst; if (d->inzero(raw)) dst = d->Zeros(64); else
            dst = d->read(args[0]);
            BaseSemantics::SValuePtr src = d->read(args[1]);
            BaseSemantics::SValuePtr bot = ops->or_(ops->and_(dst, d->NOT(wmask)), ops->and_(d->ROR(src,
           ops->number_(32, EXTR(16, 21))), wmask)); BaseSemantics::SValuePtr top; if (d->extend(raw)) top =
           d->Replicate(ops->and_(ops->shiftRight(src, ops->number_(32, EXTR(10, 15))), ops->number_(1, 1))); else top =
           dst; d->write(args[0], ops->or_(ops->and_(top, d->NOT(tmask)), ops->and_(bot, tmask)));
            */
        BaseSemantics::SValuePtr src = d->read(args[1]);
    ...
    struct IP_uxth_ubfm_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        BaseSemantics::SValuePtr src = d->read(args[1]);
        BaseSemantics::SValuePtr bot =
            ops->or_(ops->and_(dst, d->NOT(wmask)), ops->and_(d->ROR(src, ops->number_(32, EXTR(16, 21))), wmask));
        BaseSemantics::SValuePtr top;
        if(d->extend(raw))
    ...
    struct IP_movz_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result =
            ops->or_(ops->extract(result, 0, (EXTR(21, 22) << 4)),
                     ops->or_(ops->shiftLeft(d->read(args[1]), ops->number_(32, (EXTR(21, 22) << 4))),
    ...
    struct IP_orr_log_shift_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        if(d->setflags(raw)) {
          d->writeRegister(d->REG_N, ops->extract(result, result->get_width() - 1, result->get_width()));
    ...
    struct IP_asr_asrv_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result = d->ShiftReg(d->read(args[1]), d->getShiftType(raw),
                             ops->unsignedModulo(d->UInt(operand2), ops->number_(32, d->getDatasize(raw))));
        d->write(args[0], result);
      }
    };
    ...
    struct IP_sbc_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result = d->doAddOperation(operand1, operand2, d->readRegister(d->REG_C), ops->boolean_(false), n, z, c, v);
    ...
    struct IP_ngc_sbc_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        BaseSemantics::SValuePtr result;
        BaseSemantics::SValuePtr operand1 =
            d->readRegister(RegisterDescriptor(armv8_regclass_gpr, armv8_gpr_zr, 0, 32));
        BaseSemantics::SValuePtr operand2 = d->read(args[1]);
        BaseSemantics::SValuePtr n, z, c, v;
    ...
    struct IP_csel_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result =
            ops->ite(ops->isEqual(d->ConditionHolds(ops->number_(32, d->getConditionVal(raw))), ops->boolean_(true)),
                     operand1, op2);
        d->write(args[0], result);
      }
    };
    ...
    struct IP_madd_execute : P {
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        if(d->subop(raw)) {
          result =
              ops->add(d->UInt(operand3), ops->negate(ops->unsignedMultiply(d->UInt(operand1), d->UInt(operand2))));
        } else {
          result = ops->add(d->UInt(operand3), ops->unsignedMultiply(d->UInt(operand1), d->UInt(operand2)));
        }
        d->write(args[0], ops->extract(result, 0, d->getDatasize(raw) - 1 + 1));
      }
    };
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        if(d->subop(raw)) {
          result = ops->add(d->Int(operand3, true),
                            ops->negate(ops->unsignedMultiply(d->Int(operand1, true), d->Int(operand2, true))));
        } else {
          result =
              ops->add(d->Int(operand3, true), ops->unsignedMultiply(d->Int(operand1, true), d->Int(operand2, true)));
        }
    ...
    struct IP_udiv_execute : P { //
      ...
      void p(D d, Ops ops, I insn, A args, B raw) {
        ...
        result = ops->ite(ops->isEqual(d->isZero(operand2), ops->boolean_(true)), d->Zeros(64), result);
  ...
  void DispatcherARM64::iproc_init() {
  ...
  void DispatcherARM64::regcache_init() {
    if(regdict) {
  ...
  void DispatcherARM64::memory_init() {
  ...
  RegisterDescriptor DispatcherARM64::instructionPointerRegister() const { return REG_PC; }

  RegisterDescriptor DispatcherARM64::stackPointerRegister() const { return REG_SP; }

  static bool isStatusRegister(const RegisterDescriptor& reg) {
    return reg.get_major() == armv8_regclass_pstate && reg.get_minor() == 0;
  }

  RegisterDictionary::RegisterDescriptors DispatcherARM64::get_usual_registers() const {
  ...
  void DispatcherARM64::set_register_dictionary(const RegisterDictionary* regdict) {
    BaseSemantics::Dispatcher::set_register_dictionary(regdict);
    regcache_init();
  }

  void DispatcherARM64::setFlagsForResult(const BaseSemantics::SValuePtr& result,
                                          const BaseSemantics::SValuePtr& carries, bool invertCarries, size_t nbits,
  ...
  BaseSemantics::SValuePtr DispatcherARM64::parity(const BaseSemantics::SValuePtr& v) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::invertMaybe(const BaseSemantics::SValuePtr& value, bool maybe) {
    return maybe ? operators->invert(value) : value;
  }

  BaseSemantics::SValuePtr DispatcherARM64::isZero(const BaseSemantics::SValuePtr& value) {
    return operators->equalToZero(value);
  }

  BaseSemantics::SValuePtr DispatcherARM64::ConditionHolds(const BaseSemantics::SValuePtr& cond) {
    ...
    switch(((condVal & 0xF) >> 1)) {
      ...
      case 4:
        result = operators->ite(operators->isEqual(cVal, operators->number_(1, 1)),
                                operators->ite(operators->isEqual(zVal, operators->number_(1, 0)),
  ...
  BaseSemantics::SValuePtr DispatcherARM64::NOT(const BaseSemantics::SValuePtr& expr) {
    return operators->invert(expr);
  }

  void DispatcherARM64::BranchTo(const BaseSemantics::SValuePtr& target) {
    ASSERT_require(target != NULL);
    writeRegister(REG_PC, operators->extract(target, 0, 64));
  }

  BaseSemantics::SValuePtr DispatcherARM64::Zeros(const unsigned int nbits) {
    ASSERT_require(nbits > 0);
    return operators->number_(nbits, 0);
  }

  BaseSemantics::SValuePtr DispatcherARM64::SignExtend(const BaseSemantics::SValuePtr& expr, size_t newsize) {
    ASSERT_require(newsize > 0);
    return operators->signExtend(expr, newsize);
  }

  BaseSemantics::SValuePtr DispatcherARM64::ZeroExtend(const BaseSemantics::SValuePtr& expr, size_t newsize) {
    ASSERT_require(newsize > 0);
    return operators->unsignedExtend(expr, newsize);
  }

  BaseSemantics::SValuePtr DispatcherARM64::ROR(const BaseSemantics::SValuePtr& expr,
                                                const BaseSemantics::SValuePtr& amt) {
    ASSERT_not_null(amt);
    return operators->rotateRight(expr, amt);
  }

  BaseSemantics::SValuePtr DispatcherARM64::Replicate(const BaseSemantics::SValuePtr& expr) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::getBitfieldMask(int immr, int imms, int N, bool iswmask, int datasize) {
    int hsbarg = (N << 6) | (~imms);
    int len = 0;
  ...
  size_t DispatcherARM64::getRegSize(uint32_t raw) {
  ...
  size_t DispatcherARM64::ldStrLiteralAccessSize(uint32_t raw) {
  ...
  bool DispatcherARM64::inzero(uint32_t raw) {
  ...
  bool DispatcherARM64::extend(uint32_t raw) {
    switch(IntegerOps::extract2<uint32_t>(29, 30, raw)) {
      case 2:
      case 1:
        return false;
  ...
  int DispatcherARM64::op(uint32_t raw) {
    switch(IntegerOps::extract2<uint32_t>(29, 30, raw)) {
      case 0:
      case 3:
  ...
  bool DispatcherARM64::setflags(uint32_t raw) {
  ...
  int DispatcherARM64::getDatasize(uint32_t raw) {
  ...
  int DispatcherARM64::getShiftType(uint32_t raw) {
    int v10_11 = IntegerOps::extract2(10, 11, raw);

    switch(v10_11) {
      case 0:
        return static_cast<int>(ARM64::InsnProcessor::ShiftType_LSL);
      case 1:
        return static_cast<int>(ARM64::InsnProcessor::ShiftType_LSR);
      case 2:
        return static_cast<int>(ARM64::InsnProcessor::ShiftType_ASR);
      case 3:
        return static_cast<int>(ARM64::InsnProcessor::ShiftType_ROR);
      default:
        ASSERT_not_reachable("Cannot have a value greater than 3 for a 2-bit field (field: op2, bits: 10..11)!");
    }
  }

  int DispatcherARM64::getConditionVal(uint32_t raw) {
  ...
  int DispatcherARM64::opcode(uint32_t raw) {
  ...
  bool DispatcherARM64::subop(uint32_t raw) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::doAddOperation(BaseSemantics::SValuePtr a, BaseSemantics::SValuePtr b,
    ...
    BaseSemantics::SValuePtr result =
        operators->addWithCarries(a, b, invertMaybe(carryIn, invertCarries), carries /*out*/);
    setFlagsForResult(result, carries, invertCarries, a->get_width(), n, z, c, v);
    return result;
  }

  BaseSemantics::SValuePtr DispatcherARM64::effectiveAddress(SgAsmExpression* e, size_t nbits) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::readRegister(const RegisterDescriptor& reg) {
    if(reg.get_major() == armv8_regclass_gpr && reg.get_minor() == armv8_gpr_zr) {
  ...
  void DispatcherARM64::writeRegister(const RegisterDescriptor& reg, const BaseSemantics::SValuePtr& value) {
  ...
  void DispatcherARM64::write(SgAsmExpression* e, const BaseSemantics::SValuePtr& value, size_t addr_nbits /*=0*/) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::fixMemoryAddress(const BaseSemantics::SValuePtr& addr) const {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::readMemory(const BaseSemantics::SValuePtr& addr, size_t readSize) {
  ...
  void DispatcherARM64::writeMemory(const BaseSemantics::SValuePtr& addr, size_t writeSize,
  ...
  SgAsmExpression* DispatcherARM64::getWriteBackTarget(SgAsmExpression* expr) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::UInt(const BaseSemantics::SValuePtr& expr) {
    ASSERT_not_null(expr);
    return expr;
  }

  BaseSemantics::SValuePtr DispatcherARM64::ShiftReg(const BaseSemantics::SValuePtr& src, int shiftType,
                                                     const BaseSemantics::SValuePtr& amount) {
    ASSERT_not_null(amount);

    switch(static_cast<ARM64::InsnProcessor::ShiftType>(shiftType)) {
      case ARM64::InsnProcessor::ShiftType_LSL:
        return operators->shiftLeft(src, amount);
      case ARM64::InsnProcessor::ShiftType_LSR:
        return operators->shiftRight(src, amount);
      case ARM64::InsnProcessor::ShiftType_ASR:
        return operators->shiftRightArithmetic(src, amount);
      case ARM64::InsnProcessor::ShiftType_ROR:
        return operators->rotateRight(src, amount);
      default:
        ASSERT_not_reachable("Found invalid shift type for shift instruction!");
    }
  }

  BaseSemantics::SValuePtr DispatcherARM64::CountLeadingZeroBits(const BaseSemantics::SValuePtr& expr) {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::CountLeadingSignBits(const BaseSemantics::SValuePtr& expr) {
    size_t len = expr->get_width();
    BaseSemantics::SValuePtr arg =
        operators->xor_(operators->extract(expr, 1, len), operators->extract(expr, 0, len - 1));
    return CountLeadingZeroBits(arg);
  }

  BaseSemantics::SValuePtr DispatcherARM64::Int(const BaseSemantics::SValuePtr& expr, bool isUnsigned) {
    if(isUnsigned)
      return UInt(expr);
    else {
  ...
  BaseSemantics::SValuePtr DispatcherARM64::RoundTowardsZero(const BaseSemantics::SValuePtr& expr) { return expr; }
} // namespace
} // namespace
} // namespace
