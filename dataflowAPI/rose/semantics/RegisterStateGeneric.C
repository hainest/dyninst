...
namespace rose { namespace BinaryAnalysis { namespace InstructionSemantics2 { namespace BaseSemantics {

  static bool sortByOffset(const RegisterStateGeneric::RegPair& a, const RegisterStateGeneric::RegPair& b) {
  ...
  void RegisterStateGeneric::initialize_nonoverlapping(const std::vector<RegisterDescriptor>& regs,
  ...
  static bool has_null_value(const RegisterStateGeneric::RegPair& rp) { return rp.value == NULL; }

  RegisterStateGeneric::RegPairs& RegisterStateGeneric::scanAccessedLocations(const RegisterDescriptor& reg,
                                                                              RiscOperators* ops, bool markOverlapping,
                                                                              RegPairs& accessedParts /*out*/,
                                                                              RegPairs& preservedParts /*out*/) {
    ...
    BOOST_FOREACH(RegPair& regpair, pairList) {
      ...
      // If we're allowed to modify the storage locations so as to match the accessed register then mark this pair with
      // a null value so we can erase it in the next step.
      if(markOverlapping)
  ...
  SValuePtr RegisterStateGeneric::readRegister(const RegisterDescriptor& reg, const SValuePtr& dflt,
                                               RiscOperators* ops) {
    ...
    // Iterate over the storage/value pairs to figure out what parts of the register are already in existing storage
    // locations, and which parts of those overlapping storage locations are not accessed.
    ...
    RegPairs& pairList =
        scanAccessedLocations(reg, ops, adjustLocations, accessedParts /*out*/, preservedParts /*out*/);
    ...
    if(accessCreatesLocations_) {
      BOOST_FOREACH(const BitRange& newLocation, newLocations.intervals()) {
        ...
        SValuePtr newval =
            ops->extract(dflt, newLocation.least() - reg.get_offset(), newLocation.greatest() + 1 - reg.get_offset());
    ...
    if(adjustLocations) {
      // Combine all the accessed locations into a single location corresponding exactly to the return value, and split
      // those previously existing locations that partly overlap the returned location.
      ...
      // Since this is a registerRead operation, the only thing we need to write back to the state are those parts of
      // the return value that had no locations allocated originally.
  ...
  void RegisterStateGeneric::writeRegister(const RegisterDescriptor& reg, const SValuePtr& value, RiscOperators* ops) {
    ASSERT_not_null(value);
    ASSERT_require2(reg.get_nbits() == value->get_width(),
    ...
    RegPairs accessedParts;  // parts of existing overlapping locations we access
    RegPairs preservedParts; // parts of existing overlapping locations we don't access
    RegPairs& pairList = scanAccessedLocations(reg, ops, accessModifiesExistingLocations_, accessedParts /*out*/,
                                               preservedParts /*out*/);
  ...
  void RegisterStateGeneric::updateWriteProperties(const RegisterDescriptor& reg, InputOutputProperty prop) {
  ...
  void RegisterStateGeneric::erase_register(const RegisterDescriptor& reg, RiscOperators* ops) {
    ASSERT_require(reg.is_valid());
    ASSERT_not_null(ops);
    BitRange accessedLocation = BitRange::baseSize(reg.get_offset(), reg.get_nbits());
    ...
    // Look for existing registers that overlap with this register and remove them.  If the overlap was only partial,
    // then we need to eventually add the non-overlapping part back into the list.
  ...
  RegisterStateGeneric::RegPairs RegisterStateGeneric::get_stored_registers() const {
    RegPairs retval;
  ...
  bool RegisterStateGeneric::is_partly_stored(const RegisterDescriptor& desc) const {
  ...
  bool RegisterStateGeneric::is_wholly_stored(const RegisterDescriptor& desc) const {
  ...
  bool RegisterStateGeneric::is_exactly_stored(const RegisterDescriptor& desc) const {
    BOOST_FOREACH(const RegPair& pair, registers_.getOrDefault(desc)) {
      if(desc == pair.desc)
        return true;
    }
  ...
  ExtentMap RegisterStateGeneric::stored_parts(const RegisterDescriptor& desc) const {
    ExtentMap retval;
  ...
  RegisterStateGeneric::RegPairs RegisterStateGeneric::overlappingRegisters(const RegisterDescriptor& needle) const {
  ...
  bool RegisterStateGeneric::insertWriters(const RegisterDescriptor& desc, const AddressSet& writerVas) {
  ...
  void RegisterStateGeneric::eraseWriters(const RegisterDescriptor& desc, const AddressSet& writerVas) {
  ...
  void RegisterStateGeneric::setWriters(const RegisterDescriptor& desc, const AddressSet& writerVas) {
    if(writerVas.isEmpty()) {
      eraseWriters(desc);
    } else {
      BitAddressSet& parts = writers_.insertMaybeDefault(desc);
  ...
  void RegisterStateGeneric::set_latest_writer(const RegisterDescriptor& desc, rose_addr_t writer_va) {
  ...
  // [Robb P. Matzke 2015-08-07]: deprecated
  void RegisterStateGeneric::clear_latest_writer(const RegisterDescriptor& desc) { eraseWriters(desc); }
  ...
  RegisterStateGeneric::AddressSet RegisterStateGeneric::getWritersUnion(const RegisterDescriptor& desc) const {
    if(!writers_.exists(desc))
      return AddressSet();
    const BitAddressSet& parts = writers_[desc];
    BitRange where = BitRange::baseSize(desc.get_offset(), desc.get_nbits());
  ...
  RegisterStateGeneric::AddressSet RegisterStateGeneric::getWritersIntersection(const RegisterDescriptor& desc) const {
    if(!writers_.exists(desc))
      return AddressSet();
    const BitAddressSet& parts = writers_[desc];
    BitRange where = BitRange::baseSize(desc.get_offset(), desc.get_nbits());
  ...
  std::set<rose_addr_t> RegisterStateGeneric::get_latest_writers(const RegisterDescriptor& desc) const {
    AddressSet writerVas = getWritersUnion(desc);
    std::set<rose_addr_t> retval(writerVas.values().begin(), writerVas.values().end());
    return retval;
  }

  bool RegisterStateGeneric::hasPropertyAny(const RegisterDescriptor& reg, InputOutputProperty prop) const {
  ...
  bool RegisterStateGeneric::hasPropertyAll(const RegisterDescriptor& reg, InputOutputProperty prop) const {
    if(!properties_.exists(reg))
      return false;
    const BitProperties& bitProps = properties_[reg];
    BitRange where = BitRange::baseSize(reg.get_offset(), reg.get_nbits());
  ...
  InputOutputPropertySet RegisterStateGeneric::getPropertiesUnion(const RegisterDescriptor& reg) const {
    if(!properties_.exists(reg))
  ...
  InputOutputPropertySet RegisterStateGeneric::getPropertiesIntersection(const RegisterDescriptor& reg) const {
    if(!properties_.exists(reg))
      return InputOutputPropertySet();
    const BitProperties& bitProps = properties_[reg];
    BitRange where = BitRange::baseSize(reg.get_offset(), reg.get_nbits());
  ...
  bool RegisterStateGeneric::insertProperties(const RegisterDescriptor& reg, const InputOutputPropertySet& props) {
    if(props.isEmpty())
      return false;
  ...
  bool RegisterStateGeneric::eraseProperties(const RegisterDescriptor& reg, const InputOutputPropertySet& props) {
  ...
  void RegisterStateGeneric::setProperties(const RegisterDescriptor& reg, const InputOutputPropertySet& props) {
    if(props.isEmpty()) {
      eraseProperties(reg);
    } else {
      BitProperties& bitProps = properties_.insertMaybeDefault(reg);
  ...
  std::vector<RegisterDescriptor> RegisterStateGeneric::findProperties(const InputOutputPropertySet& required,
  ...
  bool RegisterStateGeneric::merge(const BaseSemantics::RegisterStatePtr& other_, RiscOperators* ops) {
    ASSERT_not_null(ops);
