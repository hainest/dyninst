...
namespace Dyninst { namespace aarch64 {
  ...
  */
  ...
  const int32_t HYPRDBG   = SYSREG | 0x00100000;  // hypervisor debug fine-grained
  const int32_t IMPLDEF   = SYSREG | 0x00110000;  // implementation defined
  const int32_t MPAM      = SYSREG | 0x00120000;  // memory partitioning and monitoring extension
  const int32_t OTHER     = SYSREG | 0x00130000;  // other system control
  const int32_t PHYSFAR   = SYSREG | 0x00140000;  // physical fault address
  const int32_t PMU       = SYSREG | 0x00150000;  // performance monitors extension
  const int32_t PSTATE    = SYSREG | 0x00160000;  // process state
  const int32_t RAS       = SYSREG | 0x00170000;  // reliability, availability, and serviceability extension (RAS)
  const int32_t RESET     = SYSREG | 0x00180000;  // reset management
  const int32_t SEC       = SYSREG | 0x00190000;  // security for access to exception levels
  const int32_t STATPROF  = SYSREG | 0x001A0000;  // statistical profiling extension
  const int32_t SYSCTL    = SYSREG | 0x001B0000;  // system control
  const int32_t SYSFLOAT  = SYSREG | 0x001C0000;  // system floating-point
  const int32_t SYSID     = SYSREG | 0x001D0000;  // system identification
  const int32_t SYSMEMORY = SYSREG | 0x001E0000;  // system memory
  const int32_t SYSMON    = SYSREG | 0x001F0000;  // system monitor
  const int32_t SYSOS     = SYSREG | 0x00200000;  // system OS lock/access/data/control
  const int32_t SYSPTR    = SYSREG | 0x00210000;  // pointer authentication
  const int32_t SYSSPR    = SYSREG | 0x00220000;  // system special-purpose
  const int32_t SYSTIMER  = SYSREG | 0x00230000;  // system timers
  const int32_t THRD      = SYSREG | 0x00240000;  // threading
  const int32_t TRACE     = SYSREG | 0x00250000;  // system trace
  const int32_t VIRT      = SYSREG | 0x00260000;  // virtualization
  ...
  DEF_REGISTER(            vbar_el3,   0 |   FULL | EXCP | SEC | Arch_aarch64, "aarch64");
